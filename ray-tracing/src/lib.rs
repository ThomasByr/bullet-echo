...
impl Ray {
    pub fn new(origin: Vector2D<f64>, dir: Vector2D<f64>) -> Ray {
        Ray { origin, dir }
    }

    pub fn point_at(&mut self, p: Vector2D<f64>) {
        let dir = p - self.origin;
        self.dir = dir.normalise();
    }

    pub fn cast(&mut self, wall: &Wall) -> Option<Vector2D<f64>> {
        let x1 = wall.p1.x;
        let y1 = wall.p1.y;
        let x2 = wall.p2.x;
        let y2 = wall.p2.y;
        ...
        if t >= 0.0 && t <= 1.0 && u >= 0.0 {
            let mut p = Vector2D::new(0.0, 0.0);
            p.x = x1 + t * (x2 - x1);
            p.y = y1 + t * (y2 - y1);
            return Some(p);
        }
    ...
    pub fn look(&mut self, walls: &Vec<Wall>) -> Option<Vector2D<f64>> {
